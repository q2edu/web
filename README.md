...
可部署到 Vercel 的移动端活动小游戏：15 秒寻找 20 个星星饼干、可选朋友邀请任务、10 秒乌萨奇连点跳跃。指定 YouTube 音乐整段循环，目标音量 100%。
...
运行前必须先初始化 Supabase，并在 `.env.local` 填入下方四个环境变量。项目不包含本机演示房间或演示成绩；本地和 Vercel 都会连接同一个真实 Supabase 后台。
